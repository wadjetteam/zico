...
async function main() {
  ...
  await prisma.evidenceRequest.createMany({
    data: [
      { id: "EVR-001", auditId: "AUD-2026-01", requirementId: "REQ-103", controlId: "CTL-003", description: "Access matrix and privileged account list for core banking DB", evidenceType: "Document", requestedFrom: "IT Ops", requestedBy: "Omar Farid", dueDate: new Date("2026-09-05") },
      { id: "EVR-002", auditId: "AUD-2026-02", requirementId: "REQ-301", controlId: "CTL-002", description: "Network segmentation diagram and WAF configuration export", evidenceType: "Config Export", requestedFrom: "Network Team", requestedBy: "Marwa Hassan", dueDate: new Date("2026-06-10") },
      { id: "EVR-003", auditId: "AUD-2026-01", requirementId: "REQ-101", controlId: "CTL-001", description: "MFA enforcement logs for sampled privileged accounts", evidenceType: "Logs", requestedFrom: "IAM Team", requestedBy: "Omar Farid", dueDate: new Date("2026-08-15") },
    ],
  });
  ...
  await prisma.auditHistoryEvent.createMany({
    data: [
      { id: "HST-001", auditId: "AUD-2026-01", user: "system", action: "AUDIT_CREATED", when: new Date("2026-08-01"), prev: null, next: "Planned", metadata: JSON.stringify({ by: "seed" }) },
      { id: "HST-002", auditId: "AUD-2026-01", user: "Omar Farid", action: "CHECKLIST_UPDATED", when: new Date("2026-08-12"), prev: "NotTested", next: "Conformity", metadata: JSON.stringify({ itemId: "CHK-001" }) },
      { id: "HST-003", auditId: "AUD-2026-01", user: "system", action: "FINDING_CREATED", when: new Date("2026-08-13"), prev: null, next: "Open", metadata: JSON.stringify({ findingId: "FND-001" }) },
    ],
  });
