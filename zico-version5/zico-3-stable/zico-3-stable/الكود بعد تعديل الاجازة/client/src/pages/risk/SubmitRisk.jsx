...
export default function SubmitRisk() {
  ...
  useEffect(() => {
    ...
    resource("users").list().then((d) => setUsers((d.items || []).filter((u) => u.is_active !== false))).catch(() => setUsers([]));
  }, []);
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="card space-y-4 p-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Risk Owner" hint="Only active users can be assigned" className="lg:col-span-2">
              <Select
                value={form.riskOwnerId}
                onChange={(e) => setField("riskOwnerId", e.target.value)}
                options={[{ value: "", label: "Select an active risk owner…" }, ...users.map((u) => ({ value: u._id, label: `${u.name || u.fullName || u.username} — ${u.email}` }))]}
              />
            </Field>
