...
import { withRiskParam, RISK_LIFECYCLE_STEPS } from "../../lib/riskLifecycle";
import RiskLifecycleStepper from "../../components/RiskLifecycleStepper";
import { formFromRisk } from "./RiskForm";
import ParameterPanel from "./ParameterPanel";
import { CATEGORIES, IMPACT_DIMENSIONS, STATUSES } from "./constants";
...
export default function ViewRisks() {
  ...
  }, [status, category, domain, severity, hasControls, methodVersion, reassessment, changeTrigger, treatment, organization]);
  ...
  const impactNames = useMemo(() => {
    const names = new Set(IMPACT_DIMENSIONS.map((dimension) => dimension.name));
  ...
  const columns = [
    ...
    { key: "status", header: "Status", render: (r) => <span className={chipClass(r.status || "Open")}>{titleCase(r.status || "Open")}</span> },
    ...
    { key: "likelihood", header: "Likelihood", className: narrow, render: (r) => <span className="chip border-sky-800/60 bg-sky-950/40 font-mono text-sky-300">{r.likelihood ?? "—"}</span> },
    ...impactNames.map(impactCol),
    { key: "overallImpact", header: "Overall Impact", className: narrow, render: (r) => <span className="chip border-gold/40 bg-gold/10 font-mono text-gold">{r.impactScore ?? r.impact ?? "—"}</span> },
      ...
      render: (r) => (
        <div className="flex flex-col gap-1.5 whitespace-nowrap">
          ...
          <div className="flex flex-wrap gap-1">
            {RISK_LIFECYCLE_STEPS.filter((step) => step.key !== "view").map((step) => (
              <Link
                key={step.key}
                to={withRiskParam(step.path, r._id)}
                className="rounded border border-line bg-ink-deep/60 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.12em] text-neutral-300 transition hover:border-gold/50 hover:text-gold"
              >
                {step.label}
              </Link>
            ))}
  ...
  const columnOrder = [
    "riskId", "title", "status", "severityLevel", "inherentScore", "inherentLevel", "residualScore", "exceedsAppetite", "overallRisk",
    "domain", "parameter", "category", "owner", "ownerTeam", "process", "subProcess", "assetSystem", "riskDate", "likelihood",
    ...impactNames.map((impact) => `impact-${impact}`), "overallImpact", "escalationPath", "description", "threat", "vulnerability", "existingControls", "linkedControls",
    "linkedExceptions", "baseline", "reassessment", "_actions",
  ];
  const orderedColumns = columnOrder.map((key) => columns.find((column) => column.key === key)).filter(Boolean);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-col items-end gap-2">
            <RiskLifecycleStepper current="view" riskId={searchParams.get("riskId") || undefined} />
