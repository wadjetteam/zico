...
export default function AttestationsTab({ policy }) {
  ...
  const load = useCallback(async () => {
    ...
    const [a, e, t, r] = await Promise.all([
      axiosApi.get(`/policies/${policy._id}/attestations`).then((r) => r.data),
      axiosApi.get(`/policies/${policy._id}/exceptions`).then((r) => r.data),
  ...
  const saveAttestation = async (e) => {
    ...
    setSaving(true);
    try {
      await axiosApi.post(`/policies/${policy._id}/attestations`, attForm);
  ...
  const saveException = async (e) => {
    ...
    setSaving(true);
    try {
      await axiosApi.post(`/policies/${policy._id}/exceptions`, excForm);
  ...
  const deleteException = async (ex) => {
    ...
    await axiosApi.delete(`/policies/${policy._id}/exceptions/${ex._id}`);
    load();
  };

  const deleteAttestation = async (at) => {
    if (!window.confirm("Delete this attestation?")) return;
    await axiosApi.delete(`/policies/${policy._id}/attestations/${at._id}`);
    load();
  };
  ...
  const bindRisk = async (e) => {
    ...
    setSaving(true);
    try {
      await axiosApi.post(`/policies/${policy._id}/exceptions/${bindTarget._id}/bind-risk`, { riskId: bindRiskId });
      setBindOpen(false);
      setBindRiskId("");
      setBindTarget(null);
      load();
  ...
  const waiveBinding = async (e) => {
    ...
    setSaving(true);
    try {
      await axiosApi.post(`/policies/${policy._id}/exceptions/${waiveTarget._id}/waive-risk-binding`, { justification: waiveJust.trim() });
      setWaiveOpen(false);
      setWaiveJust("");
      setWaiveTarget(null);
      load();
  ...
  const setExceptionStatus = async (ex, status) => {
    try {
      if (status === "Rejected") {
        ...
        await axiosApi.put(`/policies/${policy._id}/exceptions/${ex._id}`, { status, comment });
      } else {
        await axiosApi.put(`/policies/${policy._id}/exceptions/${ex._id}`, { status });
      }
      load();
    } catch (err) {
      alert(err?.response?.data?.message || err.message);
