...
export default function HierarchyTab({ policy, reload }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await apiClient.get(`/policies/${policy._id}/hierarchy`);
    setTree(Array.isArray(data) ? data : data.items || []);
    setExpanded(new Set((Array.isArray(data) ? data : data.items || []).map((n) => n._id)));
    setLoading(false);
  }, [policy._id]);
  ...
  useEffect(() => {
    ...
    apiClient.get("/policies").then((response) => {
  ...
  const saveParent = async (e) => {
    ...
    setBusy(true);
    try {
      await apiClient.put(`/policies/${policy._id}/hierarchy`, { parentPolicy: parentId });
      setSetOpen(false);
      setParentId("");
      load();
      reload();
  ...
  const saveChild = async (e) => {
    ...
    setBusy(true);
    try {
      await apiClient.put(`/policies/${childId}/hierarchy`, { parentPolicy: policy._id });
      setLinkOpen(false);
      setChildId("");
      load();
      reload();
  ...
  const unlink = async () => {
    ...
    setBusy(true);
    try {
      await apiClient.delete(`/policies/${policy._id}/hierarchy`);
      load();
      reload();
    } catch (err) {
      alert(err?.response?.data?.message || err.message);
