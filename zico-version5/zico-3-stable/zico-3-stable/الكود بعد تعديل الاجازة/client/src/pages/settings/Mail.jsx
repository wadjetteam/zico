...
export default function Mail() {
  ...
  const fetchRecent = useCallback(async () => {
    try {
      ...
      setRecentEmails(data.items || data.logs || []);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result?.connected) setMessage({ type: "success", text: "SMTP configuration saved and connection verified." });
      else setMessage({ type: "error", text: result?.lastVerifyError || "SMTP configuration saved, but the connection could not be verified." });
      await fetchRecent();
    } catch (err) {
          ...
          <div className={`card p-5 ${status.connected ? "shadow-gold" : ""}`}>
            ...
            <Field label="App password">
              <input
                ...
                placeholder="Saved securely — enter a new one only to change it"
