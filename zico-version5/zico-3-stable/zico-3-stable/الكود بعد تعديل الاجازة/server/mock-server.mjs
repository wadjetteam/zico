...
import {
  ...
  RISK_TREATMENTS, TREATMENT_CONTROLS, TREATMENT_ACTIONS, TREATMENT_EVIDENCE, TREATMENT_AUDIT_EVENTS,
...
const loadEmailConfig = async () => {
  try {
    Object.assign(EMAIL_CONFIG, JSON.parse(await fs.readFile(EMAIL_CONFIG_FILE, "utf8")));
  } catch (error) {
    if (error.code !== "ENOENT") console.error("[EMAIL] Could not load saved configuration:", error.message);
  }
};

const saveEmailConfig = async () => {
  await fs.mkdir(new URL("./data/", import.meta.url), { recursive: true });
  await fs.writeFile(EMAIL_CONFIG_FILE, JSON.stringify(EMAIL_CONFIG, null, 2), { encoding: "utf8", mode: 0o600 });
};

const loadRisks = async () => {
  try {
    const saved = JSON.parse(await fs.readFile(RISKS_FILE, "utf8"));
    if (Array.isArray(saved)) RISKS.splice(0, RISKS.length, ...saved);
  } catch (error) {
    if (error.code !== "ENOENT") console.error("[RISKS] Could not load saved risks:", error.message);
  }
};

const saveRisks = async () => {
  await fs.mkdir(new URL("./data/", import.meta.url), { recursive: true });
  await fs.writeFile(RISKS_FILE, JSON.stringify(RISKS, null, 2), { encoding: "utf8", mode: 0o600 });
};
...
const COLLECTIONS = {
  ...
  "treatments": RISK_TREATMENTS, "treatment-controls": TREATMENT_CONTROLS,
  "treatment-actions": TREATMENT_ACTIONS, "treatment-evidence": TREATMENT_EVIDENCE, "treatment-audit-events": TREATMENT_AUDIT_EVENTS,
};

const DATABASE_FILE = fileURLToPath(new URL("./data/database.json", import.meta.url));
...
const loadDatabase = async () => {
  try {
    const saved = JSON.parse(await fs.readFile(DATABASE_FILE, "utf8"));
    for (const [name, records] of Object.entries(saved.collections || {})) {
      const collection = COLLECTIONS[name];
      if (!collection || !Array.isArray(records)) continue;
      collection.splice(0, collection.length, ...records);
    }
    for (const [key, records] of Object.entries(saved.nested || {})) {
      if (Array.isArray(records)) nested.set(key, records);
    }
  } catch (error) {
    if (error.code !== "ENOENT") console.error("[DB] Could not load saved data:", error.message);
  }
};
...
const smtpConfigured = () => Boolean(EMAIL_CONFIG.enabled && EMAIL_CONFIG.smtpHost && EMAIL_CONFIG.smtpUser && EMAIL_CONFIG.smtpAppPassword && EMAIL_CONFIG.fromEmail);
const smtpTransport = () => nodemailer.createTransport({
  host: EMAIL_CONFIG.smtpHost,
  port: Number(EMAIL_CONFIG.smtpPort) || 587,
  secure: Boolean(EMAIL_CONFIG.secure) || Number(EMAIL_CONFIG.smtpPort) === 465,
  auth: { user: EMAIL_CONFIG.smtpUser, pass: EMAIL_CONFIG.smtpAppPassword },
  connectionTimeout: 3000,
  greetingTimeout: 3000,
  socketTimeout: 5000,
});
const verifySmtp = async () => {
  if (!smtpConfigured()) return { ok: false, message: "Complete SMTP host, mailbox, app password and From email first." };
  try {
...
const sendRiskOwnerNotification = async ({ risk, owner, event = "created" }) => {
  const subject = event === "created" ? `Risk assigned: ${risk.riskId} — ${risk.title}` : `Risk treatment updated: ${risk.riskId} — ${risk.title}`;
  const body = event === "created"
    ? `Hello ${owner.name || owner.fullName || owner.username},\n\nA new risk has been assigned to you in WADJET GRC.\n\nRisk: ${risk.riskId}\nTitle: ${risk.title}\nScore: ${risk.inherentScore}\nLevel: ${risk.inherentLevel}\nDue date: ${risk.targetDate || risk.deadline || "Not set"}\n\nPlease review the risk and update its treatment plan.\n`
    : `Hello ${owner.name || owner.fullName || owner.username},\n\nThe treatment plan for your risk was updated in WADJET GRC.\n\nRisk: ${risk.riskId}\nTitle: ${risk.title}\nTreatment: ${risk.treatmentDecision || "Not specified"}\nActions: ${risk.treatmentActions || risk.mitigationActions || "Not specified"}\n\nPlease review the latest treatment details.\n`;
  const log = { _id: newId("em"), to: owner.email, subject, body, status: "Failed", attachments: [], scheduledAt: null, createdAt: new Date().toISOString() };
  if (!smtpConfigured()) {
    log.error = "SMTP is not configured";
    EMAIL_MESSAGES.unshift(log);
    return { sent: false, message: log.error };
  }
  try {
    const info = await smtpTransport().sendMail({
      from: EMAIL_CONFIG.fromName ? `"${EMAIL_CONFIG.fromName}" <${EMAIL_CONFIG.fromEmail}>` : EMAIL_CONFIG.fromEmail,
      to: owner.email,
  ...
  EMAIL_MESSAGES.unshift(log);
  return { sent: log.status === "Sent", message: log.error || null };
};
...
const withJoins = (item) => {
  if (RISKS.some((r) => r._id === item._id)) {
    ...
    const owner = USERS.find((u) => u._id === out.riskOwnerId);
    if (owner) out.riskOwner = { _id: owner._id, name: owner.name || owner.fullName || owner.username, email: owner.email, department: owner.department || "", is_active: owner.is_active !== false };
    const exs = [];
    for (const p of POLICIES) {
...
const organizationIdOf = (record) => String(record.organization?._id || record.organization || "");

const syncOrganizationLinks = () => {
  const domainOrganizations = { "d-1": "o-1", "d-2": "o-1", "d-3": "o-1", "d-4": "o-2", "d-5": "o-1", "d-6": "o-3" };
  const assetOrganizations = { "a-1": "o-1", "a-2": "o-2", "a-3": "o-1", "a-4": "o-2", "a-5": "o-1", "a-6": "o-1", "a-7": "o-1", "a-8": "o-3" };
  const organization = (id) => {
    const org = ORGANIZATIONS.find((item) => item._id === id);
    return org ? { _id: org._id, name: org.name } : null;
  };
  for (const domain of DOMAINS) {
    const linked = organization(domain.organization?._id || domain.organization || domainOrganizations[domain._id]);
    if (linked) domain.organization = linked;
  }
  for (const asset of ASSETS) {
    const linked = organization(asset.organization?._id || asset.organization || assetOrganizations[asset._id]);
    if (linked) asset.organization = linked;
  }
  for (const risk of RISKS) {
    const domain = DOMAINS.find((item) => item._id === (risk.domain?._id || risk.domain));
    if (domain?.organization) risk.organization = domain.organization;
  }
};
...
const dashboardSummary = () => {
  ...
  const implemented = CONTROLS.filter((control) => control.status === "Active / Implemented" || control.progress >= 80).length;
  const compliancePercent = CONTROLS.length ? Math.round((implemented / CONTROLS.length) * 100) : 0;
  const controlEffectivenessPercent = CONTROLS.length
    ? Math.round(CONTROLS.reduce((sum, control) => sum + Number(control.effectiveness?.overall || 0), 0) / CONTROLS.length)
    : 0;
  ...
  const controlEffectivenessDistribution = [
    { name: "Effective", value: CONTROLS.filter((control) => control.effectiveness?.overall >= 80).length },
    { name: "Partially Effective", value: CONTROLS.filter((control) => control.effectiveness?.overall >= 50 && control.effectiveness?.overall < 80).length },
    { name: "Ineffective", value: CONTROLS.filter((control) => control.effectiveness?.overall > 0 && control.effectiveness?.overall < 50).length },
    { name: "Not Assessed", value: CONTROLS.filter((control) => !control.effectiveness?.overall).length },
  ];
  ...
  return {
    ...
    frameworkCompliance: FRAMEWORKS.map((f) => {
      ...
      const impl = cs.filter((c) => c.status === "Active / Implemented" || c.progress >= 80).length;
...
const findActiveUser = (id) => USERS.find((u) => u._id === id && u.is_active !== false);
...
const handle = async (req, res, url) => {
  ...
  const treatmentById = (id) => RISK_TREATMENTS.find((item) => item._id === id || item.id === id);
  const canManageTreatment = (treatment) => treatment.owner_id === user._id || ["cro", "board", "ciso"].includes(user.role);
  const normalizeTreatmentWorkflow = (treatment) => {
    const workflowMap = { DRAFT: "draft", PROPOSED: "submitted", APPROVED: "approved", IN_PROGRESS: "in_progress", COMPLETED: "completed", REJECTED: "rejected", OVERDUE: "overdue" };
    const targetDate = treatment.target_date ? new Date(treatment.target_date) : null;
    const isOverdue = !!targetDate && !["COMPLETED", "REJECTED"].includes(treatment.status) && targetDate < new Date();
    treatment.overdue_flag = Boolean(isOverdue || treatment.overdue_flag);
    if (isOverdue && treatment.status !== "OVERDUE") treatment.status = "OVERDUE";
    treatment.workflow_status = workflowMap[treatment.status] || String(treatment.status || "draft").toLowerCase();
    if (!treatment.notification_history) treatment.notification_history = [];
    if (treatment.reminder_count == null) treatment.reminder_count = 0;
    if (treatment.escalation_stage == null) treatment.escalation_stage = "initial";
    return treatment;
  };
  const auditTreatment = (treatment, action, metadata = {}) => TREATMENT_AUDIT_EVENTS.unshift({
    _id: newId("trt-audit"), treatment_id: treatment._id, action, actor_id: user._id,
    actor_role: user.role, metadata, occurred_at: new Date().toISOString(),
  });
  const sendTreatmentEmail = async ({ treatmentId, type, to, subject, body }) => {
    const log = {
      _id: newId("mail"),
    ...
    if (!to) {
      log.status = "Skipped";
      log.error = "No recipient configured";
      EMAIL_MESSAGES.unshift(log);
      return { sent: false, message: log.error };
    }

    if (!smtpConfigured()) {
      log.error = "SMTP is not configured. Set email settings first.";
      EMAIL_MESSAGES.unshift(log);
      return { sent: false, message: log.error };
    }
  ...
  const notifyTreatment = async (treatment, type, to, subject, body) => {
    const today = new Date().toISOString().slice(0, 10);
    if (EMAIL_MESSAGES.some((message) => message.treatment_id === treatment._id && message.notification_type === type && String(message.createdAt).slice(0, 10) === today)) return;
    const entry = { _id: newId("mail"), treatment_id: treatment._id, notification_type: type, to, subject, body, status: "queued", createdAt: new Date().toISOString() };
    const history = Array.isArray(treatment.notification_history) ? treatment.notification_history : [];
    const result = to ? await sendTreatmentEmail({ treatmentId: treatment._id, type, to, subject, body }) : { sent: false, message: "No recipient configured" };
    entry.status = result.sent ? "Sent" : "Failed";
    entry.error = result.message || null;
    entry.sentAt = result.sent ? new Date().toISOString() : null;
    EMAIL_MESSAGES.unshift(entry);
    history.unshift({ type, to, subject, body, createdAt: entry.createdAt, status: result.sent ? "Sent" : "Failed", error: entry.error || null });
    treatment.notification_history = history.slice(0, 20);
  };
  const treatmentView = (treatment) => {
    const normalized = normalizeTreatmentWorkflow(treatment);
    return {
      ...normalized,
      risk: RISKS.find((item) => item._id === treatment.risk_id) || null,
      owner: USERS.find((item) => item._id === treatment.owner_id) || null,
      controls: TREATMENT_CONTROLS.filter((item) => item.treatment_id === treatment._id).map((item) => ({ ...item, control: CONTROLS.find((control) => control._id === item.control_id) || null })),
      actions: TREATMENT_ACTIONS.filter((item) => item.treatment_id === treatment._id),
      evidence: TREATMENT_EVIDENCE.filter((item) => item.treatment_id === treatment._id),
      audit_events: TREATMENT_AUDIT_EVENTS.filter((item) => item.treatment_id === treatment._id),
      workflow_status: normalized.workflow_status,
      overdue_flag: Boolean(normalized.overdue_flag),
      notification_history: normalized.notification_history || [],
    };
  };

  if (path === "v1/treatments" && method === "GET") return json(res, 200, { items: RISK_TREATMENTS.map(treatmentView), total: RISK_TREATMENTS.length });
  if (parts.length === 3 && parts[0] === "v1" && parts[1] === "treatments" && method === "GET") {
    const treatment = treatmentById(parts[2]);
    return treatment ? json(res, 200, treatmentView(treatment)) : json(res, 404, { message: "Treatment not found" });
  }

  // ISO/IEC 27005 risk-treatment API. Controls are deliberately selected only
  // from the stored ISO 27002 library: no AI/RAG recommendation endpoint exists.
  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "risks" && parts[3] === "treatments" && method === "POST") {
    const risk = RISKS.find((item) => item._id === parts[2]);
    if (!risk) return json(res, 404, { message: "Risk not found" });
    const body = await readBody(req);
    const strategies = ["MODIFY", "RETAIN", "AVOID", "SHARE"];
    const likelihood = Number(body.residual_likelihood);
    const impact = Number(body.residual_impact);
    if (!strategies.includes(body.strategy)) return json(res, 400, { message: "A valid treatment strategy is required" });
    if (!String(body.justification || "").trim()) return json(res, 400, { message: "Justification is required" });
    if (!Number.isInteger(likelihood) || likelihood < 1 || likelihood > 5 || !Number.isInteger(impact) || impact < 1 || impact > 5) {
      return json(res, 400, { message: "Residual likelihood and impact must be integers from 1 to 5" });
    }
    const residualScore = likelihood * impact;
    const inherentScore = Number(risk.riskScore || risk.inherentScore || 0);
    if (residualScore > inherentScore) return json(res, 400, { message: "Residual score cannot exceed inherent score" });
    const controlIds = Array.isArray(body.control_ids) ? [...new Set(body.control_ids)] : [];
    if (body.strategy === "MODIFY" && !controlIds.length) return json(res, 400, { message: "At least one ISO 27002 control is required" });
    if (controlIds.some((id) => !CONTROLS.some((control) => control._id === id))) return json(res, 400, { message: "One or more selected controls are invalid" });
    if (body.strategy === "SHARE" && !body.vendor_id) return json(res, 400, { message: "A third-party vendor is required for a share treatment" });
    const requiresCommitteeApproval = residualScore > 9;
    const treatment = {
      _id: newId("trt"), id: newId("trt-id"), risk_id: risk._id, strategy: body.strategy,
      justification: String(body.justification).trim(), residual_likelihood: likelihood, residual_impact: impact,
      residual_score: residualScore, status: requiresCommitteeApproval ? "PROPOSED" : "DRAFT",
      owner_id: body.owner_id || user._id, target_date: body.target_date || null,
      allocated_budget: Number(body.allocated_budget || 0), progress_percentage: 0,
      vendor_id: body.vendor_id || null, attachment_name: body.attachment_name || null,
      asset_id: body.asset_id || risk.asset?._id || null, created_by: user._id, createdAt: new Date().toISOString(),
      workflow_status: requiresCommitteeApproval ? "submitted" : "draft",
      overdue_flag: false,
    ...
    RISK_TREATMENTS.unshift(treatment);
    controlIds.forEach((control_id) => TREATMENT_CONTROLS.push({ treatment_id: treatment._id, control_id, source: "MANUAL", implementation_status: "PLANNED" }));
    auditTreatment(treatment, "created", { strategy: treatment.strategy, residual_score: treatment.residual_score });
    if (body.strategy === "AVOID" && treatment.asset_id) {
      const asset = ASSETS.find((item) => item._id === treatment.asset_id);
      if (asset) asset.status = "Inactive";
    }
    if (requiresCommitteeApproval) await notifyTreatment(treatment, "approval-request", "cro@wadjet.local", "Risk treatment requires formal approval", `Treatment ${treatment._id} for ${risk.riskId} has residual score ${residualScore}.`);
    return json(res, 201, { ...treatmentView(treatment), approval_required: requiresCommitteeApproval });
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "submit" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!canManageTreatment(treatment)) return json(res, 403, { message: "Not authorized to submit this treatment" });
    if (treatment.status !== "DRAFT") return json(res, 409, { message: "Only a draft treatment can be submitted" });
    treatment.status = "PROPOSED"; treatment.workflow_status = "submitted"; treatment.submitted_at = new Date().toISOString();
    treatment.notification_history = Array.isArray(treatment.notification_history) ? treatment.notification_history : [];
    auditTreatment(treatment, "submitted");
    await notifyTreatment(treatment, "approval-request", "cro@wadjet.local", "Risk treatment awaiting approval", `Treatment ${treatment._id} is ready for approval.`);
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "acknowledge" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!canManageTreatment(treatment)) return json(res, 403, { message: "Not authorized to acknowledge this treatment" });
    treatment.acknowledged_at = new Date().toISOString();
    treatment.acknowledged_by = user._id;
    treatment.last_reminder_at = treatment.last_reminder_at || treatment.acknowledged_at;
    auditTreatment(treatment, "acknowledged", { acknowledged_by: user._id });
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "reminder" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!canManageTreatment(treatment)) return json(res, 403, { message: "Not authorized to send reminders" });
    const nextLevel = Number(treatment.reminder_count || 0) + 1;
    treatment.reminder_count = nextLevel;
    treatment.escalation_stage = nextLevel >= 3 ? "committee" : nextLevel >= 2 ? "manager" : "owner";
    treatment.last_reminder_at = new Date().toISOString();
    const target = USERS.find((item) => item._id === treatment.owner_id)?.email || "risk.manager@wadjet.local";
    const message = nextLevel >= 3 ? "Escalated to committee follow-up for overdue risk treatment" : `Follow-up reminder for treatment ${treatment._id}`;
    await notifyTreatment(treatment, "deadline-reminder", target, message, `Reminder ${nextLevel} sent for treatment ${treatment._id}.`);
    const due = treatment.target_date ? new Date(treatment.target_date) : null;
    if (due && due < new Date()) {
      treatment.overdue_flag = true;
      treatment.status = "OVERDUE";
      treatment.workflow_status = "overdue";
      treatment.escalation_stage = "manager";
    }
    auditTreatment(treatment, "reminder-sent", { reminder_count: nextLevel, escalation_stage: treatment.escalation_stage });
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "decision" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!["cro", "board", "ciso"].includes(user.role)) return json(res, 403, { message: "Only an authorized approver may decide a treatment" });
    const body = await readBody(req);
    if (treatment.status !== "PROPOSED") return json(res, 409, { message: "Only proposed treatments may be approved or rejected" });
    if (!["APPROVED", "REJECTED"].includes(body.decision) || !String(body.comment || "").trim()) return json(res, 400, { message: "Decision and decision comment are required" });
    if (treatment.owner_id === user._id) return json(res, 403, { message: "Segregation of duties prevents an owner approving their own treatment" });
    treatment.status = body.decision; treatment.workflow_status = body.decision === "APPROVED" ? "approved" : "rejected"; treatment.decision_comment = String(body.comment).trim(); treatment.decided_by = user._id; treatment.decided_at = new Date().toISOString();
    if (body.decision === "APPROVED") treatment.escalation_stage = "owner";
    auditTreatment(treatment, body.decision.toLowerCase(), { comment: treatment.decision_comment });
    await notifyTreatment(treatment, `decision-${body.decision.toLowerCase()}`, USERS.find((item) => item._id === treatment.owner_id)?.email || treatment.owner_id, `Risk treatment ${body.decision.toLowerCase()}`, treatment.decision_comment);
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "actions" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!canManageTreatment(treatment)) return json(res, 403, { message: "Not authorized to manage actions" });
    const body = await readBody(req);
    if (!String(body.title || "").trim() || !body.owner_id || !body.target_date) return json(res, 400, { message: "Action title, owner and target date are required" });
    const action = { _id: newId("trt-action"), treatment_id: treatment._id, title: String(body.title).trim(), owner_id: body.owner_id, target_date: body.target_date, dependency: body.dependency || null, budget: Number(body.budget || 0), status: "PLANNED", progress_percentage: 0, created_at: new Date().toISOString() };
    TREATMENT_ACTIONS.push(action); auditTreatment(treatment, "action-added", { action_id: action._id });
    return json(res, 201, action);
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "evidence" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!canManageTreatment(treatment)) return json(res, 403, { message: "Not authorized to add evidence" });
    const body = await readBody(req);
    if (!String(body.name || "").trim() || !String(body.reference || "").trim()) return json(res, 400, { message: "Evidence name and immutable reference are required" });
    const evidence = { _id: newId("trt-evidence"), treatment_id: treatment._id, name: String(body.name).trim(), reference: String(body.reference).trim(), evidence_type: body.evidence_type || "implementation", uploaded_by: user._id, uploaded_at: new Date().toISOString() };
    TREATMENT_EVIDENCE.push(evidence); auditTreatment(treatment, "evidence-added", { evidence_id: evidence._id });
    return json(res, 201, evidence);
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "effectiveness-review" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!["cro", "ciso", "board"].includes(user.role)) return json(res, 403, { message: "Independent risk or security review is required" });
    const body = await readBody(req);
    if (treatment.owner_id === user._id) return json(res, 403, { message: "Segregation of duties prevents an owner reviewing their own treatment" });
    if (TREATMENT_EVIDENCE.filter((item) => item.treatment_id === treatment._id).length === 0) return json(res, 400, { message: "Implementation evidence is required before effectiveness review" });
    if (!body.control_effectiveness || Object.values(body.control_effectiveness).some((score) => Number(score) < 80)) return json(res, 400, { message: "All implemented controls require effectiveness evidence of at least 80%" });
    treatment.effectiveness_review = { reviewed_by: user._id, reviewed_at: new Date().toISOString(), control_effectiveness: body.control_effectiveness, residual_likelihood: treatment.residual_likelihood, residual_impact: treatment.residual_impact, residual_score: treatment.residual_score };
    auditTreatment(treatment, "effectiveness-reviewed");
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "close-monitoring" && method === "POST") {
    const treatment = treatmentById(parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (!["cro", "ciso", "board"].includes(user.role)) return json(res, 403, { message: "CISO/CRO closure approval is required" });
    const body = await readBody(req); const risk = RISKS.find((item) => item._id === treatment.risk_id); const appetite = Number(body.appetite_limit ?? 8);
    if (treatment.status !== "COMPLETED" || !treatment.effectiveness_review || treatment.residual_score > appetite || Number(body.green_kri_quarters) < 4 || Number(body.related_incidents) !== 0) return json(res, 400, { message: "Closure monitoring requires completed treatment, effectiveness review, residual risk within appetite, four green KRI quarters, and zero related incidents" });
    risk.status = "CLOSED_MONITORING"; risk.closedMonitoringAt = new Date().toISOString(); treatment.monitoring_approved_by = user._id; auditTreatment(treatment, "closed-monitoring", { appetite });
    return json(res, 200, treatmentView(treatment));
  }

  if (parts.length === 4 && parts[0] === "v1" && parts[1] === "treatments" && parts[3] === "progress" && method === "PATCH") {
    const treatment = RISK_TREATMENTS.find((item) => item._id === parts[2] || item.id === parts[2]);
    if (!treatment) return json(res, 404, { message: "Treatment not found" });
    if (treatment.owner_id !== user._id && !["board", "cro"].includes(user.role)) return json(res, 403, { message: "Only the treatment owner may update progress" });
    if (!["APPROVED", "IN_PROGRESS"].includes(treatment.status)) return json(res, 409, { message: "Progress can be updated only after treatment approval" });
    const body = await readBody(req);
    const progress = Number(body.progress_percentage);
    if (!Number.isInteger(progress) || progress < 0 || progress > 100) return json(res, 400, { message: "Progress must be an integer from 0 to 100" });
    treatment.progress_percentage = progress;
    treatment.status = progress === 100 ? "COMPLETED" : "IN_PROGRESS";
    treatment.workflow_status = progress === 100 ? "completed" : "in_progress";
    treatment.updatedAt = new Date().toISOString();
    auditTreatment(treatment, "progress-updated", { progress });
    return json(res, 200, treatmentView(treatment));
  }
  ...
  if (path === "users" && method === "POST") {
    ...
    if (!name || !email) return json(res, 422, { message: "name and email are required" });
    if (USERS.some((u) => u.email.toLowerCase() === email.toLowerCase())) return json(res, 409, { message: "A user with this email already exists" });
    const username = String(body.username || email.split("@")[0]).trim();
    const created = { _id: newId("user"), username, fullName: name, name, email, department: body.department || "", is_active: body.is_active !== false, role: body.role || "risk_owner" };
    USERS.unshift(created);
    return json(res, 201, created);
  }
  if (parts.length === 2 && parts[0] === "users" && method === "PUT") {
    const target = USERS.find((u) => u._id === parts[1]);
    if (!target) return json(res, 404, { message: "User not found" });
    const body = await readBody(req);
    if (body.name !== undefined || body.fullName !== undefined) {
      target.name = String(body.name || body.fullName || target.name || target.fullName).trim();
      target.fullName = target.name;
    }
    if (body.email !== undefined) target.email = String(body.email).trim();
    if (body.department !== undefined) target.department = String(body.department).trim();
  ...
  if ((path === "risks/preview-score" || path === "risks/calculate") && method === "POST") {
    const body = await readBody(req);
    ...
    if (!Object.keys(criteriaScores).length) {
      for (const i of normalizeImpacts(body.impacts)) criteriaScores[i.name] = i.value;
    }

    // Build risk controls from linkedControls
    const riskControls = (body.riskControls || body.linkedControls || []).map((c) => ({
  ...
  if (parts.length === 3 && parts[0] === "risks" && parts[2] === "notify-owner" && method === "POST") {
    const risk = RISKS.find((r) => r._id === parts[1]);
    if (!risk) return json(res, 404, { message: "Risk not found" });
    const owner = findActiveUser(risk.riskOwnerId);
    if (!owner?.email) return json(res, 422, { message: "This risk has no active owner with an email address." });
    const result = await sendRiskOwnerNotification({ risk, owner });
    if (!result.sent) return json(res, 502, { message: `Risk owner email failed: ${result.message}` });
    return json(res, 200, { ok: true, to: owner.email, message: "Risk owner notification sent." });
  }
  ...
  if (path === "risks" && method === "POST") {
    ...
    const riskOwnerId = body.riskOwnerId || user._id;
    if (!findActiveUser(riskOwnerId)) return json(res, 422, { message: "riskOwnerId must reference an active user" });
    const domainId = body.domain?._id || body.domain || "d-4";
    const domain = DOMAINS.find((d) => d._id === domainId);
    const param = PARAMETERS.find((p) => String(p.domain?._id) === String(domainId));
    if (!param) return json(res, 400, { message: "No active parameter found for this domain." });
    ...
    RISKS.push(next);
    await saveRisks();
    const owner = findActiveUser(next.riskOwnerId);
    const emailNotification = owner?.email
      ? await sendRiskOwnerNotification({ risk: next, owner })
      : { sent: false, message: "The assigned active user has no email address." };
    return json(res, 200, { ...next, emailNotification });
  }
  ...
  if (parts.length === 2 && parts[0] === "risks" && method === "PUT") {
    ...
    const body = await readBody(req);
    if (body.computed) return json(res, 400, { message: "Computed fields are managed by the server and cannot be provided by the client." });
    if (body.riskOwnerId && !findActiveUser(body.riskOwnerId)) return json(res, 422, { message: "riskOwnerId must reference an active user" });
    ...
    if (body.riskOwnerId) risk.riskOwnerId = body.riskOwnerId;
  ...
  if (path === "assets/stats" && method === "GET") {
    const scopedAssets = query.organization ? ASSETS.filter((a) => organizationIdOf(a) === String(query.organization)) : ASSETS;
    const byCriticality = {};
    const byStatus = {};
    const byType = {};
    ...
    return json(res, 200, { total: scopedAssets.length, byCriticality, byStatus, byType });
  }
  ...
  if (path.startsWith("context/organizations/")) {
    ...
    if (rest.length === 2 && rest[1] === "hierarchy") {
      ...
      const parentIdOf = (item) => String(item.parentOrg?._id || item.parentOrg || "");
      const buildTree = (item, visited = new Set()) => {
        if (visited.has(item._id)) return null;
        const nextVisited = new Set(visited).add(item._id);
        const children = ORGANIZATIONS
          .filter((candidate) => parentIdOf(candidate) === item._id)
          .map((candidate) => buildTree(candidate, nextVisited))
          .filter(Boolean);
        return { ...item, children };
      };
      return json(res, 200, { items: ORGANIZATIONS, tree: buildTree(org) });
    }
    if (rest.length === 2 && rest[1] === "rollup") {
      const organizationId = rest[0];
      const risks = RISKS.filter((r) => organizationIdOf(r) === organizationId);
      const assets = ASSETS.filter((a) => organizationIdOf(a) === organizationId);
      const domains = DOMAINS.filter((d) => organizationIdOf(d) === organizationId);
      return json(res, 200, { counts: { risks: risks.length, assets: assets.length, domains: domains.length, organizations: ORGANIZATIONS.filter((o) => o._id === organizationId).length, controls: CONTROLS.length } });
    }
  }
  ...
  if (path === "email/status") {
    ...
    return json(res, 200, {
      ...
      smtpHost: EMAIL_CONFIG.smtpHost || EMAIL_CONFIG.host || "",
      smtpPort: EMAIL_CONFIG.smtpPort || EMAIL_CONFIG.port || 587,
      smtpUser: EMAIL_CONFIG.smtpUser || EMAIL_CONFIG.user || "",
      fromEmail: EMAIL_CONFIG.fromEmail || "",
      fromName: EMAIL_CONFIG.fromName || "Wadjet GRC",
      lastVerifyError: verification.message,
      lastTestAt: null,
    });
  }
  if (path === "email/recent") return json(res, 200, { items: EMAIL_MESSAGES.slice(0, 5) });
  if (path === "email/config") {
    if (method === "GET") return json(res, 200, { config: { ...EMAIL_CONFIG, smtpAppPassword: undefined, hasAppPassword: Boolean(EMAIL_CONFIG.smtpAppPassword) } });
    if (method === "PUT") {
      const body = await readBody(req);
      Object.assign(EMAIL_CONFIG, body, {
        smtpHost: body.smtpHost || EMAIL_CONFIG.smtpHost || body.host,
        smtpPort: Number(body.smtpPort || EMAIL_CONFIG.smtpPort || body.port || 587),
        smtpUser: body.smtpUser || EMAIL_CONFIG.smtpUser || body.user,
        fromEmail: body.fromEmail || EMAIL_CONFIG.fromEmail,
        fromName: body.fromName || EMAIL_CONFIG.fromName || "Wadjet GRC",
      });
      if (!body.smtpAppPassword) EMAIL_CONFIG.smtpAppPassword = EMAIL_CONFIG.smtpAppPassword || "";
      EMAIL_CONFIG.enabled = true;
      await saveEmailConfig();
      return json(res, 200, { ok: true, config: { ...EMAIL_CONFIG, smtpAppPassword: undefined, hasAppPassword: Boolean(EMAIL_CONFIG.smtpAppPassword) } });
    }
  }
  if (path === "email/disconnect" && method === "POST") { EMAIL_CONFIG.enabled = false; await saveEmailConfig(); return json(res, 200, { ok: true }); }
  if (path === "email/test" && method === "POST") {
    const to = String((await readBody(req)).to || "").trim();
    if (!to) return json(res, 422, { message: "A recipient email is required." });
    const verification = await verifySmtp();
    if (!verification.ok) return json(res, 422, { message: verification.message });
    try {
      const info = await smtpTransport().sendMail({
        from: EMAIL_CONFIG.fromName ? `"${EMAIL_CONFIG.fromName}" <${EMAIL_CONFIG.fromEmail}>` : EMAIL_CONFIG.fromEmail,
        to,
        replyTo: EMAIL_CONFIG.replyTo || undefined,
        subject: "WADJET GRC SMTP test",
        text: "This is a test email from WADJET GRC.",
      });
      EMAIL_MESSAGES.unshift({ _id: newId("em"), to, subject: "WADJET GRC SMTP test", body: "This is a test email from WADJET GRC.", status: "Sent", sentAt: new Date().toISOString(), createdAt: new Date().toISOString() });
      return json(res, 200, { ok: true, messageId: info.messageId });
    } catch (error) {
      return json(res, 502, { message: `SMTP send failed: ${error.message}` });
    }
  }
  ...
  // Inbox: expose locally-generated report packages (search up to 6 parent dirs for report-package-*.zip)
  if (path === "reports/inbox" && method === "GET") {
    async function findReportZips() {
      const items = [];
      ...
      for (let i = 0; i < 6; i++) {
        try {
          ...
          for (const name of names) {
            if (/^report-package.*\.zip$/i.test(name)) {
              const filePath = fileURLToPath(new URL(name, dirUrl));
              let stat = null;
              try { stat = await fs.stat(filePath); } catch (e) { stat = null; }
              items.push({ name, path: filePath, size: stat ? stat.size : null, mtime: stat ? stat.mtime.toISOString() : null });
            }
          }
        } catch (e) {
          // ignore permission/read errors
        }
        dirUrl = new URL("../", dirUrl);
      }
      return items;
    }

    const found = await findReportZips();
    const items = found.map((f) => ({ id: f.name.replace(/[^a-z0-9\-_.]/gi, "_"), name: f.name, size: f.size, createdAt: f.mtime, downloadUrl: `/api/reports/download?name=${encodeURIComponent(f.name)}` }));
    return json(res, 200, { items, total: items.length });
  }

  if (path === "reports/download" && method === "GET") {
    const name = String(query.name || "").trim();
    if (!name) return json(res, 400, { message: "name query param is required" });
    async function findReportFileByName(target) {
      let dirUrl = new URL("./", import.meta.url);
      for (let i = 0; i < 6; i++) {
        try {
          const candidate = fileURLToPath(new URL(target, dirUrl));
          try { await fs.access(candidate); return candidate; } catch (e) { /* not found here */ }
        } catch (e) { }
        dirUrl = new URL("../", dirUrl);
      }
      return null;
    }
    const filePath = await findReportFileByName(name);
    if (!filePath) return json(res, 404, { message: "Report file not found" });
    try {
      const stat = await fs.stat(filePath);
      res.writeHead(200, { "Content-Type": "application/zip", "Content-Disposition": `attachment; filename="${name}"`, "Content-Length": stat.size });
      const stream = fsSync.createReadStream(filePath);
      stream.pipe(res);
      return;
    } catch (err) {
      return json(res, 500, { message: `Unable to read file: ${err.message}` });
    }
  }
...
if (parts.length === 5 && parts[0] === "governance" && parts[1] === "roles" && parts[3] === "users") {
  ...
  if (parts.length === 2 && parts[0] === "policies" && method === "GET" && !LIFECYCLE_SUBS.includes(parts[1]) && parts[1] !== "dashboard" && parts[1] !== "stats") {
    try {
      ...
      const policy = POLICIES.find((item) => item._id === parts[1]);
      const lifecycleStateByStatus = { Draft: "DRAFT", Review: "REVIEW", Approval: "APPROVAL", Approved: "APPROVED", Published: "PUBLISHED", Active: "ACTIVE", Archived: "ARCHIVED" };
      const lifecycleState = lifecycleStateByStatus[policy?.status] || summary.lifecycleState;
      return json(res, 200, { ...policy, ...summary, lifecycleState, effectiveState: lifecycleState, title: policy?.title || policy?.name || summary.policyNumber || parts[1] });
    } catch (err) {
  ...
  // Get audit history
  if (parts.length === 3 && parts[0] === "policies" && parts[2] === "audit-logs" && method === "GET") {
    const nestedEntries = getNested(`policies::${parts[1]}::audit-logs`).map((entry) => ({
      ...entry,
      timestamp: entry.createdAt,
    }));
    const governanceEntries = GOVERNANCE_AUDIT_LOG.filter((entry) => entry.policyId === parts[1]).map((entry) => ({
      ...entry,
      actionType: entry.action || entry.actionType,
      actor: entry.actorUserId || entry.actor,
      actorRole: entry.actorRoleAtTime || entry.actorRole,
      createdAt: entry.timestamp || entry.createdAt,
      details: entry.newValue || entry.details,
    }));
    return json(res, 200, { items: [...nestedEntries, ...governanceEntries], total: nestedEntries.length + governanceEntries.length });
  }
  ...
  if (path === "reports/generate" && method === "GET") {
    ...
    try {
      ...
      const dashboardSnapshot = dashboardSummary();
      const buffer = await generateReport(reportId, format, null, user, { dashboardSummary: dashboardSnapshot });
      logReportGeneration(user, reportId, format, def.dataSource ? (await def.dataSource({ dashboardSummary: dashboardSnapshot })).length : 0);
  ...
  if (collection) {
    if (method === "GET") {
      try {
        const scoped = query.organization
          ? collection.filter((record) => organizationIdOf(record) === String(query.organization))
          : collection;
        const { items, total, page, pageSize } = filterList(scoped, query);
    ...
    if (method === "POST") {
      const body = await readBody(req);
      const generatedId = path === "organizations"
        ? `ORG-${String(collection.length + 1).padStart(3, "0")}`
        : path === "domains"
          ? `DMN-${String(collection.length + 1).padStart(3, "0")}`
          : null;
      const rec = { _id: newId("rec"), ...body, createdAt: new Date().toISOString() };
      if (path === "organizations" && !rec.orgId) rec.orgId = generatedId;
      if (path === "domains" && !rec.domainId) rec.domainId = generatedId;
  ...
  if (parts.length >= 2) {
    ...
    if (idx >= 0) {
      ...
      if (method === "PUT") {
        const body = await readBody(req);
        if (parts[0] === "risks" && String(body.status).toUpperCase() === "CLOSED") {
          const completedTreatment = RISK_TREATMENTS.find((treatment) => treatment.risk_id === id && treatment.monitoring_approved_by);
          if (!completedTreatment) return json(res, 409, { message: "Risk closure requires an approved treatment effectiveness review and Closed Monitoring approval" });
          if (!["cro", "ciso", "board"].includes(user.role)) return json(res, 403, { message: "Only CISO, CRO, or Board may formally close a risk" });
          const monitoringSince = new Date(collection2[idx].closedMonitoringAt || 0);
          const quartersElapsed = (Date.now() - monitoringSince.getTime()) / (91 * 86400000);
          if (!Number.isFinite(quartersElapsed) || quartersElapsed < 4) return json(res, 409, { message: "Risk must remain in Closed Monitoring for four consecutive quarters before formal closure" });
        }
...
await loadRisks();
await loadDatabase();
syncOrganizationLinks();

const runTreatmentFollowUp = () => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  for (const treatment of RISK_TREATMENTS) {
    if (!treatment.target_date || ["COMPLETED", "REJECTED"].includes(treatment.status)) continue;
    const due = new Date(treatment.target_date); due.setHours(0, 0, 0, 0);
    const daysRemaining = Math.ceil((due - today) / 86400000);
    const owner = USERS.find((item) => item._id === treatment.owner_id);
    if (treatment.progress_percentage < 100 && daysRemaining <= 7 && daysRemaining >= 0) {
      notifyTreatment(treatment, "deadline-reminder", owner?.email || treatment.owner_id, "Risk treatment deadline approaching", `Treatment ${treatment._id} is due in ${daysRemaining} day(s).`);
    }
    if (daysRemaining < 0) {
      treatment.overdue = true;
      treatment.overdue_since = treatment.overdue_since || new Date().toISOString();
      notifyTreatment(treatment, "overdue-escalation", "risk.manager@wadjet.local", "Overdue risk treatment", `Treatment ${treatment._id} is overdue and remains ${treatment.status}.`);
    }
  }
};
...
http
  .createServer((req, res) => {
    ...
    handle(req, res, url).then(() => {
      if (mutating) return saveDatabase().catch((error) => console.error("[DB] Could not save data:", error.message));
    }).catch((err) => {
      console.error(err);
  ...
  .listen(PORT, "0.0.0.0", () => {
    ...
    runTreatmentFollowUp();
    setInterval(runTreatmentFollowUp, 24 * 60 * 60 * 1000).unref();
