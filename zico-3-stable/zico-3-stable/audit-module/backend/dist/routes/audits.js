...
function generateAuditCode(count) {
    return `AUD-${new Date().getFullYear()}-${String(count + 1).padStart(2, "0")}`;
}
// List all audits
exports.auditsRouter.get("/", async (req, res, next) => {
    try {
        ...
        const total = await index_1.prisma.audit.count({ where });
        const items = await index_1.prisma.audit.findMany({
            where,
...
exports.auditsRouter.get("/:id", async (req, res, next) => {
    try {
        const audit = await index_1.prisma.audit.findUnique({
        ...
        if (!audit)
            throw (0, errorHandler_1.notFound)("Audit not found");
        res.json(audit);
    }
    catch (err) {
        next(err);
...
// Create audit
exports.auditsRouter.post("/", async (req, res, next) => {
    try {
        ...
        const count = await index_1.prisma.audit.count();
        const audit = await index_1.prisma.audit.create({
            data: {
...
// Update audit
exports.auditsRouter.put("/:id", async (req, res, next) => {
    try {
        const existing = await index_1.prisma.audit.findUnique({ where: { id: req.params.id } });
        if (!existing)
            throw (0, errorHandler_1.notFound)("Audit not found");
        ...
        const audit = await index_1.prisma.audit.update({
            where: { id: req.params.id },
            data: {
...
exports.auditsRouter.patch("/:id/status", async (req, res, next) => {
    try {
        const { status } = req.body;
        if (!status)
            throw (0, errorHandler_1.badRequest)("Status is required");
        const existing = await index_1.prisma.audit.findUnique({
            where: { id: req.params.id },
        ...
        if (status === "Completed") {
            ...
            if (untestedCount > 0) {
                throw (0, errorHandler_1.badRequest)(`Cannot complete audit with ${untestedCount} untested checklist item(s)`);
            }
        }
        const audit = await index_1.prisma.audit.update({
            where: { id: req.params.id },
        ...
        // Log history
        await index_1.prisma.auditHistoryEvent.create({
            data: {
                auditId: audit.id,
                user: "system",
...
exports.auditsRouter.get("/:id/overall-result", async (req, res, next) => {
    try {
        const audit = await index_1.prisma.audit.findUnique({
            where: { id: req.params.id },
        ...
        if (!audit)
            throw (0, errorHandler_1.notFound)("Audit not found");
        const result = computeOverallResult(audit.checklistItems, audit.findings);
...
function computeOverallResult(checklistItems, findings) {
    ...
    if (tested.length < applicable.length)
        return "Not Conclusive";
    ...
    if (openCritical || ratio > 0.4)
        return "Ineffective";
    if (ratio > 0.15)
        return "Partially Effective";
    return "Effective";
}
exports.default = { computeOverallResult };
