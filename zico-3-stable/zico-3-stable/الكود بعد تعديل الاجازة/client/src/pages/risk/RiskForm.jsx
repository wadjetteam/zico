...
export default function RiskForm({
  ...
  useEffect(() => {
    ...
    api
      .get("/users")
      .then((r) => {
        if (active) setUsers((r.data?.items || r.data || []).filter((user) => user.is_active !== false));
      })
      .catch(() => {});
    return () => {
      active = false;
  ...
  return (
    <div className="space-y-6">
      <Section title="Identification">
        ...
        <Field label="Risk Owner" hint="Only active users can be assigned">
          <Select
            value={form.riskOwnerId || ""}
            onChange={set("riskOwnerId")}
            options={[{ value: "", label: "Select an active risk owner" }, ...users.map((user) => ({ value: user._id, label: `${user.name || user.fullName || user.username} — ${user.email}` }))]}
          />
        </Field>
      </Section>
