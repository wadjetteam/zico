...
export default function WorkflowTab({ policy, reload }) {
  ...
  useEffect(() => {
    apiClient.get(`/policies/${policy._id}/audit-logs`).then((r) => setHistory((r.data.items || []).filter((l) => APPROVAL_ACTIONS.includes(l.actionType)))).catch(() => {});
  }, [policy._id]);
  ...
  const run = async (action, confirmMsg, extra = {}) => {
    ...
    setBusy(action);
    try {
      await apiClient.post(`/policies/${policy._id}/workflow`, { action, ...extra });
      reload();
    } catch (err) {
  ...
  const reject = async (e) => {
    ...
    setBusy("reject");
    try {
      await apiClient.post(`/policies/${policy._id}/workflow`, { action: "reject", comment: rejectComment.trim() });
      setRejectOpen(false);
      setRejectComment("");
      reload();
    } catch (err) {
  ...
  const saveStages = async () => {
    setBusy("stages");
    try {
      await apiClient.put(`/policies/${policy._id}`, { workflowStages: stages.filter((s) => s.name.trim()) });
