...
export default function VersionsTab({ policy, reload, compareOnly }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiClient.get(`/policies/${policy._id}/versions`).then((r) => setRows(r.data.items || [])).finally(() => setLoading(false));
  }, [policy._id]);
  ...
  const save = async (e) => {
    ...
    setSaving(true);
    try {
      await apiClient.post(`/policies/${policy._id}/versions`, { changeType: form.changeType, summary: form.changeSummary });
      setOpen(false);
  ...
  const remove = async (row) => {
    ...
    await apiClient.delete(`/policies/${policy._id}/versions/${row._id}`);
    load();
  };
