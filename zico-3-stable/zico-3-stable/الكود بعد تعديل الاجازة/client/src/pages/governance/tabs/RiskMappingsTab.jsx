...
export default function RiskMappingsTab({ policy }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiClient.get(`/policies/${policy._id}/risk-mappings`).then((r) => setRows(r.data.items || [])).finally(() => setLoading(false));
  }, [policy._id]);

  useEffect(() => { load(); }, [load]);

  useEffect(() => {
    apiClient.get("/risks").then((r) => setRiskOptions(r.data.items || []));
  }, []);
  ...
  const save = async (e) => {
    ...
    setSaving(true);
    try {
      await apiClient.post(`/policies/${policy._id}/risk-mappings`, form);
      setOpen(false);
  ...
  const remove = async (row) => {
    ...
    await apiClient.delete(`/policies/${policy._id}/risk-mappings/${row._id}`);
    load();
  };
