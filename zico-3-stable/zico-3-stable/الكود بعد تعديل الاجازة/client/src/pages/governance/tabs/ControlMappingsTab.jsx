...
export default function ControlMappingsTab({ policy }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiClient.get(`/policies/${policy._id}/control-mappings`).then((r) => setRows(r.data.items || [])).finally(() => setLoading(false));
  }, [policy._id]);

  useEffect(() => { load(); }, [load]);

  useEffect(() => {
    Promise.all([apiClient.get("/frameworks"), apiClient.get("/controls")]).then(([frameworks, controls]) => {
      setFrameworkOptions(frameworks.data.items || []);
      setControlOptions(controls.data.items || []);
    });
  }, []);

  const controls = useMemo(() => controlOptions.filter((control) => {
    if (!form.framework) return true;
    return (control.frameworkMappings || []).some((mapping) => mapping.framework?._id === form.framework || mapping.framework?.name === form.framework);
  }).map((control) => ({
    id: String(control._id || control.controlId),
    controlId: control.controlId || control._id,
    name: control.name || control.title || control.controlId,
  })), [controlOptions, form.framework]);
  ...
  const save = async (e) => {
    ...
    try {
      ...
      await apiClient.post(`/policies/${policy._id}/control-mappings`, {
        controlId: ctrl?.controlId || form.control,
  ...
  const remove = async (row) => {
    ...
    await apiClient.delete(`/policies/${policy._id}/control-mappings/${row._id}`);
    load();
  };
    ...
    <div>
      ...
      <DataTable
        columns={[
          ...
          { key: "controlName", header: "Control name", render: (r) => <Link to="/controls/management" className="font-medium text-gold hover:text-gold-light">{orDash(r.controlName)}</Link> },
