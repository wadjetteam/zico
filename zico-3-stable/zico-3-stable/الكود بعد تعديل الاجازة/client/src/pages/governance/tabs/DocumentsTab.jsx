...
export default function DocumentsTab({ policy }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    apiClient.get(`/policies/${policy._id}/documents`).then((r) => setRows(r.data.items || [])).finally(() => setLoading(false));
  }, [policy._id]);
    ...
    try {
      ...
      if (editing === "new") {
        if (file) {
          ...
          await apiClient.post(`/policies/${policy._id}/documents`, fd, { headers: { "Content-Type": "multipart/form-data" } });
        } else {
          await apiClient.post(`/policies/${policy._id}/documents`, payload);
        }
      } else {
        await apiClient.put(`/policies/${policy._id}/documents/${editing._id}`, payload);
      }
      setEditing(null);
      setFile(null);
      load();
  ...
  const remove = async (row) => {
    ...
    await apiClient.delete(`/policies/${policy._id}/documents/${row._id}`);
    load();
  };
