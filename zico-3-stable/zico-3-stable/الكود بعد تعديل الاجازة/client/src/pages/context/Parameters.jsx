...
import { Eye, Layers, Loader2, Pencil, Plus, RefreshCw, RotateCcw, SlidersHorizontal, Trash2 } from "lucide-react";
    ...
    <>
      ...
      {error ? (
        ...
        <DataTable
          ...
          toolbar={
            <>
              <select className="input w-56" value={domainFilter} onChange={(e) => { setDomainFilter(e.target.value); const next = new URLSearchParams(searchParams); if (e.target.value) next.set("domain", e.target.value); else next.delete("domain"); setSearchParams(next, { replace: true }); }}>
          ...
          columns={[
            ...
            },
            {
              ...
              render: (r) => {
                ...
                return (
                  <div>
                    <Link to={`/risk/view?domain=${encodeURIComponent(String(r.domain?._id || r.domain || ""))}`} className="text-neutral-200 hover:text-gold">{d?.name || "—"}</Link>
            ...
            },
            {
              ...
              render: (r) => (
                <div className="flex justify-end gap-1">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); openScale(r, "likelihoodScale"); }} className="rounded-md p-1.5 text-neutral-500 transition hover:bg-white/5 hover:text-gold" title="Edit scales (likelihood & impact)">
      ...
      <Modal
        ...
        title={viewing ? `Parameter details — ${viewing.name || "Parameter"}` : "Parameter details"}
        subtitle="Read-only summary of the selected scoring parameter."
        width="max-w-2xl"
        footer={
          <>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-1 text-neutral-200">{viewing.active ? "Active" : "Inactive"}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Domain</p>
                <p className="mt-1 text-neutral-200">{domainName.get(String(viewing.domain?._id || viewing.domain))?.name || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Score method</p>
                <p className="mt-1 text-neutral-200">{METHOD_LABELS[viewing.riskScoreMethod || "multiplicative"]}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded border border-line bg-white/[0.02] p-3">
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Critical</p>
                <p className="mt-1 font-mono text-neutral-100">{viewing.thresholds?.critical ?? "—"}</p>
              </div>
              <div className="rounded border border-line bg-white/[0.02] p-3">
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">High</p>
                <p className="mt-1 font-mono text-neutral-100">{viewing.thresholds?.high ?? "—"}</p>
              </div>
              <div className="rounded border border-line bg-white/[0.02] p-3">
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Medium</p>
                <p className="mt-1 font-mono text-neutral-100">{viewing.thresholds?.medium ?? "—"}</p>
              </div>
              <div className="rounded border border-line bg-white/[0.02] p-3">
                <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Appetite</p>
                <p className="mt-1 font-mono text-neutral-100">{viewing.appetiteLimit ?? "—"}</p>
              </div>
            </div>

            <div>
              <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Impact criteria</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {(viewing.criteria || []).map((c) => (
                  <span key={`${c.name}-${c.weight}`} className="chip !py-0.5 text-[11px]">
                    {c.name} <span className="font-mono text-[10px] text-neutral-500">{Number(c.weight).toFixed(2)}</span>
                  </span>
                ))}
              </div>
            ...
            <div>
              <p className="text-[11px] uppercase tracking-[0.14em] text-neutral-500">Scales</p>
              <p className="mt-1 text-neutral-200">
                Likelihood: {(viewing.likelihoodScale || []).length} levels · Impact: {(viewing.impactScale || []).length} levels
              </p>
            </div>
          </div>
        )}
