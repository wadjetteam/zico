...
import {
  ...
  Link2, Loader2, MapPin, Network, Pencil, ShieldCheck, Tag,
} from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="mt-5">
        ...
        {tab === "records" && <LinkedRecordsTab counts={rollup?.counts} organizationId={id} />}
      </div>
    </div>
  );
}
...
function LinkedRecordsTab({ counts, organizationId }) {
  const navigate = useNavigate();
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4">
      {RECORD_LINKS.map((r) => {
        ...
        const scopedPath = ["domains", "assets", "risks"].includes(r.key) ? `${r.path}?organization=${organizationId}` : r.path;
        return (
          <button key={r.key} onClick={() => navigate(scopedPath)} className="card flex items-center gap-4 px-5 py-4 text-left transition hover:border-gold/40">
